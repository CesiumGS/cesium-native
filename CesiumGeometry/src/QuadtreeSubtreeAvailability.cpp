...
static constexpr uint64_t countOnesInBuffer(
...
static constexpr uint16_t getMortonIndexForBytes(uint8_t x, uint8_t y) {
  return 
...
static constexpr uint32_t getMortonIndexForShorts(uint16_t x, uint16_t y) {
...
static constexpr uint64_t getMortonIndexForInts(uint32_t x, uint32_t y) {
...
QuadtreeSubtreeAvailability::Subtree::Subtree(
  ...
  : levels(levels_),
    ...
    this->contentAvailability = 
        gsl::span<const std::byte>(
          reinterpret_cast<const std::byte*>(bitstream.data() + tileAvailabilitySize),
          tileAvailabilitySize);
    this->subtreeAvailability = 
        gsl::span<const std::byte>(
...
QuadtreeSubtreeAvailability::Node::Node(Subtree&& subtree_) 
  ...
  size_t childNodesCount = 
      countOnesInBuffer(this->subtree.subtreeAvailability);
...
bool QuadtreeSubtreeAvailability::addSubtree(
    const QuadtreeTileID& tileID, Subtree&& newSubtree) noexcept {
  ...
  if (tileID.level == 0) {
    if (this->_pRoot) {
      ...
      this->_pRoot = std::make_unique<Node>(newSubtree);
