...
EXT_mesh_gpu_instancingJsonHandler::EXT_mesh_gpu_instancingJsonHandler(
    const ReaderContext& context) noexcept
    : ExtensibleObjectJsonHandler(context), _attributes() {}

void EXT_mesh_gpu_instancingJsonHandler::reset(
    CesiumJsonReader::IJsonHandler* pParentHandler,
    EXT_mesh_gpu_instancing* pObject) {
  ExtensibleObjectJsonHandler::reset(pParentHandler, pObject);
  this->_pObject = pObject;
}

CesiumJsonReader::IJsonHandler*
EXT_mesh_gpu_instancingJsonHandler::readObjectKey(const std::string_view& str) {
  assert(this->_pObject);
  return this->readObjectKeyEXT_mesh_gpu_instancing(
      EXT_mesh_gpu_instancing::TypeName,
      str,
      *this->_pObject);
}
...
void EXT_mesh_gpu_instancingJsonHandler::reset(
    ...
    const std::string_view& extensionName) {
  std::any& value =
      o.extensions.emplace(extensionName, EXT_mesh_gpu_instancing())
          .first->second;
  this->reset(pParentHandler, &std::any_cast<EXT_mesh_gpu_instancing&>(value));
}

CesiumJsonReader::IJsonHandler*
EXT_mesh_gpu_instancingJsonHandler::readObjectKeyEXT_mesh_gpu_instancing(
    const std::string& objectType,
    const std::string_view& str,
