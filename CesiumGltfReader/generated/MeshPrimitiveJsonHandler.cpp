...
IJsonHandler* MeshPrimitiveJsonHandler::MeshPrimitiveKey(const char* str, MeshPrimitive& o) {
  ...
  if ("extensions"s == str) return property("extensions", this->_extensions, o.extensions);

  return this->ExtensibleObjectKey(str, *this->_pObject);
}

void MeshPrimitiveJsonHandler::ExtensionsJsonHandler::reset(IJsonHandler* pParent, std::vector<std::any>* pExtensions) {
    ObjectJsonHandler::reset(pParent);
    this->_pExtensions = pExtensions;
}

IJsonHandler* MeshPrimitiveJsonHandler::ExtensionsJsonHandler::Key(const char* str, size_t /* length */, bool /* copy */) {
    using namespace std::string_literals;

    if ("KHR_draco_mesh_compression"s == str) return property("KHR_draco_mesh_compression", this->_KHR_draco_mesh_compression, std::any_cast<KHR_draco_mesh_compression&>(this->_pExtensions->emplace_back(KHR_draco_mesh_compression())));
