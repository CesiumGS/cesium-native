...
const std::vector<std::byte> TRIANGLE_INDICES_THEN_FLOATS {
    std::byte(0x00), std::byte(0x00), std::byte(0x01), std::byte(0x00), std::byte(0x02), std::byte(0x00), std::byte(0x00), std::byte(0x00), 
    std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), 
    std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x80), std::byte(0x3f), 
    std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), 
    std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x00), std::byte(0x80), std::byte(0x3f), 
...
Model generateTriangleModel() {
    ...
    buffer.byteLength = static_cast<std::int64_t>(TRIANGLE_INDICES_THEN_FLOATS.size());
...
TEST_CASE("Basic triangle is serialized to embedded glTF 2.0", "[GltfWriter]") {
    ...
    const auto validateStructure = [](const std::vector<std::byte>& gltfAsset) {
        ...
        REQUIRE(accessors[0].componentType == CesiumGltf::AccessorSpec::ComponentType::UNSIGNED_SHORT);
        ...
        REQUIRE(accessors[1].componentType == CesiumGltf::AccessorSpec::ComponentType::FLOAT);
        ...
        REQUIRE((indicesBufferView.target.has_value() && *indicesBufferView.target == CesiumGltf::BufferView::Target::ELEMENT_ARRAY_BUFFER));

        const auto positionBufferView = bufferViews.at(1); REQUIRE(positionBufferView.buffer == 0);
        REQUIRE(positionBufferView.byteOffset == 8);
        REQUIRE(positionBufferView.byteLength == 36);
        REQUIRE((positionBufferView.target.has_value() && *positionBufferView.target == CesiumGltf::BufferView::Target::ARRAY_BUFFER));
    ...
    const auto asGltfBytes = CesiumGltf::writeModelAsEmbeddedBytes(model, WriteFlags::GLTF | CesiumGltf::AutoConvertDataToBase64);
    const auto asGLBBytes = CesiumGltf::writeModelAsEmbeddedBytes(model, WriteFlags::GLB);
