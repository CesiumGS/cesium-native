...
[[nodiscard]] std::string mimeTypeToMimeString(CesiumGltf::Image::MimeType mimeType) noexcept {
...
[[nodiscard]] std::string mimeTypeToExtensionString(CesiumGltf::Image::MimeType mimeType) noexcept {
    if (mimeType == CesiumGltf::Image::MimeType::image_jpeg) {
        return ".jpeg";
    } else {
        return ".png";
    ...
    for (auto i = 0ul; i < images.size(); ++i) {
        ...
        if (isBase64URI) {
            if (!isDataBufferEmpty) {
                throw AmbiguiousDataSource("image.uri cannot be a base64 uri if image.cesium.pixelData is non-empty");
            }
            ...
            if (!isDataBufferEmpty) {
                throw MissingDataSource("image.uri references an external file, but image.cesium.pixelData is empty");
            }
            ...
            if (flags & WriteFlags::AutoConvertDataToBase64) {
                j.KeyPrimitive("uri", BASE64_PREFIX + encodeAsBase64String(image.cesium.pixelData));
            } 

            // Automatically generate a uri using the mimetype / index and call
            // the user provided lambda.
            else {
                const auto ext = image.mimeType ? mimeTypeToExtensionString(*image.mimeType) : "";
