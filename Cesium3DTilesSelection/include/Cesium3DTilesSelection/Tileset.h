 ...
 * datasets.
 */
class CESIUM3DTILESSELECTION_API Tileset final : public ITilesetFrameInfo {
public:
  /**
   ...
   */
  std::optional<CesiumUtility::Credit> getUserCredit() const noexcept override;

  /**
   * @brief Get tileset credits.
   */
  const std::vector<CesiumUtility::Credit>&
  getTilesetCredits() const noexcept override;
  ...
  const RasterOverlayCollection& getOverlays() const noexcept override;
  ...
  virtual bool getEnableLodTransitionPeriod() const override {
    return this->getOptions().enableLodTransitionPeriod;
  }

  virtual const std::shared_ptr<CesiumUtility::CreditSystem>&
  getCreditSystem() const override {
