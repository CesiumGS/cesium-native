...
void createImplicitQuadtreeLoader(
  ...
  if (holds_alternative<CesiumGeometry::BoundingSphere>(boundingVolume)) {
    return;
  }
  ...
  const CesiumGeometry::OrientedBoundingBox* pBox =
      get_if<CesiumGeometry::OrientedBoundingBox>(&boundingVolume);
  const CesiumGeospatial::S2CellBoundingVolume* pS2Cell =
      get_if<CesiumGeospatial::S2CellBoundingVolume>(&boundingVolume);
...
void createImplicitOctreeLoader(
  ...
  if (holds_alternative<CesiumGeospatial::S2CellBoundingVolume>(
          boundingVolume)) {
    return;
  }
...
TilesetJsonLoader::loadTileContent(const TileLoadInput& loadInput) {
  ...
  const std::string* url = get_if<std::string>(&tile.getTileID());
  if (!url) {
