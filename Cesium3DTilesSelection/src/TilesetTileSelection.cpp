...
void TilesetTileSelection::_updateLodTransitions(
...
void TilesetTileSelection::_frustumCull(
    const Tile& tile,
    const TilesetFrameState& frameState,
  ...
  const CesiumGeospatial::Ellipsoid& ellipsoid = this->_options.ellipsoid;
...
void TilesetTileSelection::_fogCull(
    const TilesetFrameState& frameState,
...
bool TilesetTileSelection::_meetsSse(
    const std::vector<ViewState>& frustums,
    const Tile& tile,
    const std::vector<double>& distances,
...
TilesetTileSelection::TraversalDetails TilesetTileSelection::_visitTileIfNeeded(
    const TilesetFrameState& frameState,
    uint32_t depth,
  ...
  if (this->hasCullingFlag(TileSelectionCullingFlags::ChildBounds)) {
  ...
  if (this->hasCullingFlag(TileSelectionCullingFlags::Excluder)) {
  ...
  if (this->hasCullingFlag(TileSelectionCullingFlags::Frustum)) {
    this->_frustumCull(tile, frameState, cullWithChildrenBounds, cullResult);
  }

  if (this->hasCullingFlag(TileSelectionCullingFlags::Fog)) {
  ...
  if (!cullResult.shouldVisit) {
    ...
    if (this->_options.forbidHoles && tile.getRefine() == TileRefine::Replace) {
      ...
      traversalDetails =
          TilesetTileSelection::createTraversalDetailsForSingleTile(
              frameState,
              tile);
...
TilesetTileSelection::TraversalDetails TilesetTileSelection::_renderLeaf(
    const TilesetFrameState& frameState,
    Tile& tile,
    double tilePriority,
    ViewUpdateResult& result) {
  ...
  return TilesetTileSelection::createTraversalDetailsForSingleTile(
      frameState,
      tile);
}
...
TilesetTileSelection::TraversalDetails TilesetTileSelection::_renderInnerTile(
    const TilesetFrameState& frameState,
    Tile& tile,
    ViewUpdateResult& result) {
  addCurrentTileDescendantsToTilesFadingOutIfPreviouslyRendered(
...
bool TilesetTileSelection::_loadAndRenderAdditiveRefinedTile(
    const TilesetFrameState& frameState,
    Tile& tile,
    ViewUpdateResult& result,
    double tilePriority,
...
bool TilesetTileSelection::_kickDescendantsAndRenderTile(
    const TilesetFrameState& frameState,
    Tile& tile,
    ViewUpdateResult& result,
    TraversalDetails& traversalDetails,
...
TileOcclusionState TilesetTileSelection::_checkOcclusion(const Tile& tile) {
  const std::shared_ptr<TileOcclusionRendererProxyPool>& pOcclusionPool =
      this->_pTilesetContentManager->getExternals().pTileOcclusionProxyPool;
  if (pOcclusionPool) {
...
//   * The tile may or may not be renderable.
//   * The tile has not yet been added to a load queue.
TilesetTileSelection::TraversalDetails TilesetTileSelection::_visitTile(
    const TilesetFrameState& frameState,
    uint32_t depth,
    bool meetsSse,
  ...
  bool shouldCheckOcclusion =
      ...
      this->hasCullingFlag(TileSelectionCullingFlags::Occlusion) &&
...
TilesetTileSelection::TraversalDetails
TilesetTileSelection::_visitVisibleChildrenNearToFar(
    const TilesetFrameState& frameState,
    uint32_t depth,
    bool ancestorMeetsSse,
    Tile& tile,
...
void TilesetTileSelection::addTileToLoadQueue(
    const TilesetFrameState& frameState,
    Tile& tile,
  ...
  frameState.viewGroup.addToLoadQueue(
      ...
      this->_pTilesetContentManager->getExternals().pGltfModifier);
}
...
bool TilesetTileSelection::hasCullingFlag(TileSelectionCullingFlags flag) {
  return ((uint32_t)this->_cullingFlags & (uint32_t)flag) == (uint32_t)flag;
}

const ViewUpdateResult& TilesetTileSelection::updateViewGroup(
  ...
  this->_pTilesetContentManager->registerTileRequester(viewGroup);
  ...
  if (!frustums.empty()) {
    viewGroup.startNewFrame(tilesetFrameInfo);
    this->_visitTileIfNeeded(frameState, 0, false, *pRootTile, result);
    viewGroup.finishFrame(tilesetFrameInfo);
  } else {
