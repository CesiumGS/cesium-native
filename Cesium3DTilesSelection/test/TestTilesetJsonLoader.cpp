...
TEST_CASE("Test creating tileset json loader") {
  ...
  SECTION("Create valid tileset json with REPLACE refinement") {
    ...
    const auto* pRegion =
        get_if<CesiumGeospatial::BoundingRegion>(&boundingVolume);
    ...
    CHECK(get<std::string>(children[0].getTileID()) == "ll.b3dm");
    CHECK(holds_alternative<CesiumGeospatial::BoundingRegion>(
    ...
    CHECK(get<std::string>(children[1].getTileID()) == "lr.b3dm");
    CHECK(holds_alternative<CesiumGeospatial::BoundingRegion>(
    ...
    CHECK(get<std::string>(children[2].getTileID()) == "ur.b3dm");
    CHECK(holds_alternative<CesiumGeospatial::BoundingRegion>(
    ...
    CHECK(get<std::string>(children[3].getTileID()) == "ul.b3dm");
    CHECK(holds_alternative<CesiumGeospatial::BoundingRegion>(
  ...
  SECTION("Create valid tileset json with ADD refinement") {
    ...
    for (const Tile& child : pRootTile->getChildren()) {
      ...
      CHECK(get<std::string>(child.getTileID()) == *expectedUrlIt);
      CHECK(holds_alternative<CesiumGeospatial::BoundingRegion>(
  ...
  SECTION("Tileset has tile with sphere bounding volume") {
    ...
    const CesiumGeometry::BoundingSphere& sphere =
        get<CesiumGeometry::BoundingSphere>(rootTile.getBoundingVolume());
  ...
  SECTION("Tileset with quadtree implicit tile") {
    ...
    CHECK(
        get<CesiumGeometry::QuadtreeTileID>(child.getTileID()) ==
  ...
  SECTION("Tileset with octree implicit tile") {
    ...
    CHECK(child.getTransform() == pRootTile->getTransform());
    CHECK(
        get<CesiumGeometry::OctreeTileID>(child.getTileID()) ==
...
TEST_CASE("Test loading individual tile of tileset json") {
  ...
  SECTION("Load tile that has render content") {
    ...
    const auto& tileID = get<std::string>(pRootTile->getTileID());
  ...
  SECTION("Load tile that has external content") {
    ...
    pRootTile->getContent().setContentKind(
        std::make_unique<TileExternalContent>(
            get<TileExternalContent>(tileLoadResult.contentKind)));
    ...
    CHECK(get<std::string>(parentB3dmTile.getTileID()) == "parent.b3dm");
    ...
    for (std::size_t i = 0; i < parentB3dmChildren.size(); ++i) {
      ...
      CHECK(get<std::string>(child.getTileID()) == expectedChildUrls[i]);
  ...
  SECTION("Load tile that has external content with implicit tiling") {
    ...
    const auto& tileID =
        get<CesiumGeometry::QuadtreeTileID>(implicitTile.getTileID());
...

    {
      ...
      auto implicitContentResult = implicitContentResultFuture.wait();
      CHECK(holds_alternative<CesiumGltf::Model>(
