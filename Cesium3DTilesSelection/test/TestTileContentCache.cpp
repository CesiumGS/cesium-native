  ...
  loadTileContent(const TileLoadInput& loadInput) override {
    ...
    return loadInput.pTileContentCache->getOrLoad(
        ...
        *std::get_if<std::string>(&loadInput.tile.getTileID()),
        ...
        std::move(mockLoadInWorkerThread));
  }

  virtual TileChildrenResult createTileChildren(const Tile& /*tile*/) override {
    TileChildrenResult result{std::move(this->mockChildren), TileLoadResultState::Success};
...
TEST_CASE("Test tile content cache") {
  ...
  auto pMockedAssetAccessor = std::make_shared<SimpleAssetAccessor>(std::move(mockedRequests));

  // The cache starts empty.
  auto pMockedCacheAssetAccessor = 
      std::make_shared<SimpleCachedAssetAccessor>(
  ...
  SECTION("Loading, unloading, and reloading causes cache hit.") {
    ...
    REQUIRE(pMockedCacheAssetAccessor->mockCache.find("a.com") != pMockedCacheAssetAccessor->mockCache.end());
    ...
    // Since there was derived data in the cache, loading should have been skipped.
...
TEST_CASE("Test Mocked SimpleCachedAssetAccessor") { 
  ...
  SECTION("Test write-through") {
    auto pRequestA = cachedAssetAccessor.get(asyncSystem, "a.com", {}, true).wait();
    auto pRequestB = cachedAssetAccessor.get(asyncSystem, "b.com", {}, true).wait();
    ...
    REQUIRE(cachedAssetAccessor.mockCache.find("a.com") 
              != cachedAssetAccessor.mockCache.end());
    REQUIRE(cachedAssetAccessor.mockCache.find("b.com") 
              != cachedAssetAccessor.mockCache.end());
  ...
  SECTION("Test write-back") {
    auto pRequestA = cachedAssetAccessor.get(asyncSystem, "a.com", {}, false).wait();
    auto pRequestB = cachedAssetAccessor.get(asyncSystem, "b.com", {}, false).wait();
    auto pRequestC = cachedAssetAccessor.get(asyncSystem, "c.com", {}, true).wait();
    ...
    REQUIRE(cachedAssetAccessor.mockCache.find("c.com") 
              != cachedAssetAccessor.mockCache.end());
    ...
    cachedAssetAccessor.writeBack(asyncSystem, pRequestA, false, std::vector<std::byte>(a_clientData));
    cachedAssetAccessor.writeBack(asyncSystem, pRequestB, true, std::vector<std::byte>(b_clientData));
