 ...
 */
struct CESIUM3DTILES_API Extension3dTilesMetadataClassPropertyLegacy final
    : public CesiumUtility::ExtensibleObject {
  static inline constexpr const char* TypeName =
      "Extension3dTilesMetadataClassPropertyLegacy";
   ...
   * `componentCount` number of elements. `normalized` property has no effect on
   * these values: they always correspond to the integer values.
   */
   ...
   * `STRING` use a JSON string. For numeric types use a JSON number. For `ENUM`
   * use the enum `name`, not the integer value. For `ARRAY` use a JSON array
   * containing values matching the `componentType`.
   */
