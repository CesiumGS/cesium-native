...
RasterMappedTo3DTile::update(Tile& tile) {
  if (this->getState() == AttachmentState::Attached) {
    for (const RasterToCombine& rasterToCombine : this->_rastersToCombine) {
      if (!rasterToCombine._originalFailed && rasterToCombine._pReadyTile) {
        ...
        if (provider && 
            provider
              ->hasMoreDetailsAvailable(rasterToCombine._pReadyTile->getID())) {
  ...
  for (RasterToCombine& rasterToCombine : this->_rastersToCombine) {
    ...
    while (pLoadingTile && pLoadingTile->getState() ==
...
/*
    ...
    if (pReadyTile &&
      ...
      externals.pPrepareRendererResources->attachRasterInMainThread(
          ...
          *pReadyTile,
          ...
          this->getScale());
      * /
      // TODO: change attachment states to make more sense after the current changes?
      this->_state = pLoadingTile ? AttachmentState::TemporarilyAttached
  ...
  if (this->allReady() && !this->anyLoading() && pGeometryRectangle && this->_state == AttachmentState::Unattached) {
    for (RasterToCombine& rasterToCombine : this->_rastersToCombine) {
      this->computeTranslationAndScale(rasterToCombine, tile);
    }
    // is it safe to just assume all the rasters come from the same overlay?
    RasterOverlay& overlay = this->_rastersToCombine[0]._pReadyTile->getOverlay();
    CesiumGeometry::Rectangle geometryRectangle = 
        projectRectangleSimple(overlay.getTileProvider()->getProjection(), *pGeometryRectangle);
    if (!this->_combinedTile) {
      this->_combinedTile = 
          std::make_shared<RasterOverlayTile>(
            overlay,
            tile.getTileID(),
            geometryRectangle);
      // remove?
...
/* THINK MORE ABOUT HOW TO KNOW WHEN TO DO THIS
    // Unattach the old tile
    if (this->_combinedTile && this->getState() != AttachmentState::Unattached) {
      TilesetExternals& externals = tile.getTileset()->getExternals();
      externals.pPrepareRendererResources->detachRasterInMainThread(
          tile,
          this->getTextureCoordinateID(),
...
*/
    ...
    // fix race condition on _combinedTile (use RasterOverlayTileProvider::LoadResult)
    tile.getTileset()->getAsyncSystem().runInWorkerThread(
      [this,
       ...
       combinedTile = this->_combinedTile]() {
        ...
        if (image) {
          ...
          combinedTile->_pRendererResources = 
              externals.pPrepareRendererResources->prepareRasterInLoadThread(*image);
        } else {
      ...
      [this,
       &tile, 
       &externals = tile.getTileset()->getExternals()](std::shared_ptr<RasterOverlayTile>&& combinedTile) {       
        // Unattach the old tile
        /*
        if (combinedTile && this->getState() != AttachmentState::Unattached) {
          externals.pPrepareRendererResources->detachRasterInMainThread(
              tile,
              this->getTextureCoordinateID(),
              *combinedTile,
        ...
        this->_state = this->anyLoading() ? AttachmentState::TemporarilyAttached
                                          : AttachmentState::Attached;
        ...
        [this, &tile](const std::exception& /*e*/) {
...
// Will need a well thought out interface to let cesium-native use GPU resources while being 
// engine-agnostic.
// TODO: probably can simplify dramatically by ignoring cases where there is discrepancy between
// channels count or bytesPerChannel between the rasters
std::optional<CesiumGltf::ImageCesium> RasterMappedTo3DTile::blitRasters() {
  ...
  int32_t channels = 1;
  for (const RasterToCombine& rasterToCombine : this->_rastersToCombine) {
    const CesiumGltf::ImageCesium& rasterImage = rasterToCombine._pReadyTile->getImage();
  ...
  for (int32_t j = 0; j < image.height; ++j) {
    double v = double(j) / double(image.height);
    ...
    for (int32_t i = 0; i < image.width; ++i) {
      ...
      for (const RasterToCombine& rasterToCombine : this->_rastersToCombine) {  
        ...
        if (1 + rasterToCombine._textureCoordinateRectangle.contains(uv)) {
          const CesiumGltf::ImageCesium& srcImage = 
              rasterToCombine._pReadyTile->getImage();

          glm::dvec2 srcUv = 
              //(uv - rasterToCombine._translation / rasterToCombine._scale) * rasterToCombine._scale;
        ...
        /*
          uint32_t dbgCol = static_cast<uint8_t>(glm::floor(srcUv.x * 255.0)) << 24 | 
                            static_cast<uint8_t>(glm::floor(srcUv.y * 255.0)) << 16 |
                            255;
