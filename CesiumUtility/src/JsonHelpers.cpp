...
namespace CesiumUtility {
	...
	int32_t JsonHelpers::getInt32OrDefault(const rapidjson::Value& json, const std::string& key, int32_t defaultValue) {
		auto it = json.FindMember(key.c_str());
		return it == json.MemberEnd() ? defaultValue : JsonHelpers::getInt32OrDefault(it->value, defaultValue);
	}

	int32_t JsonHelpers::getInt32OrDefault(const rapidjson::Value& json, int32_t defaultValue) {
		if (json.IsInt()) {
			return json.GetInt();
		} else {
			return defaultValue;
	...
	uint64_t JsonHelpers::getUint64OrDefault(const rapidjson::Value& json, const std::string& key, uint64_t defaultValue) {
		auto it = json.FindMember(key.c_str());
		return it == json.MemberEnd() ? defaultValue : JsonHelpers::getUint64OrDefault(it->value, defaultValue);
	}

	uint64_t JsonHelpers::getUint64OrDefault(const rapidjson::Value& json, uint64_t defaultValue) {
		if (json.IsUint()) {
			return json.GetUint();
		} else {
			return defaultValue;
	...
	int64_t JsonHelpers::getInt64OrDefault(const rapidjson::Value& json, const std::string& key, int64_t defaultValue) {
		auto it = json.FindMember(key.c_str());
		return it == json.MemberEnd() ? defaultValue : JsonHelpers::getInt64OrDefault(it->value, defaultValue);
	}

	int64_t JsonHelpers::getInt64OrDefault(const rapidjson::Value& json, int64_t defaultValue) {
		if (json.IsInt()) {
			return json.GetInt();
		} else {
			return defaultValue;
	...
	bool JsonHelpers::getBoolOrDefault(const rapidjson::Value& json, const std::string& key, bool defaultValue) {
		auto it = json.FindMember(key.c_str());
		return it == json.MemberEnd() ? defaultValue : JsonHelpers::getBoolOrDefault(it->value, defaultValue);
	}

	bool JsonHelpers::getBoolOrDefault(const rapidjson::Value& json, bool defaultValue) {
		if (json.IsBool()) {
			return json.GetBool();
		} else {
			return defaultValue;
