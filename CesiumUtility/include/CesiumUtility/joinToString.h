...
namespace CesiumUtility {
    /**
     * @brief Joins multiple elements together into a string, separated by a given separator.
     * 
    ...
    std::string joinToString(TIterator begin, TIterator end, const std::string& separator) {
        return std::accumulate(begin, end, std::string(), [&separator](const std::string& acc, const std::string& element) {
            if (!acc.empty()) {
    ...
    std::string joinToString(TCollection collection, const std::string& separator) {
        return std::accumulate(collection.cbegin(), collection.cend(), std::string(), [&separator](const std::string& acc, const std::string& element) {
            if (!acc.empty()) {
                return acc + separator + element;
            } else {
                return element;
