...
function generate(options, schema, writers) {
  ...
  const className = `${name}${thisConfig.toBeInherited ? "Spec" : ""}`;
  ...
  const header = `
        ...
        namespace ${namespace} {
             ...
             */
            struct ${namespace.toUpperCase()}_API ${className}${thisConfig.isBaseClass ? "" : " final"} : public ${base} {
                ...
                ${className}(${className}&& rhs) noexcept = default;
                ${className}(const ${className}& rhs) noexcept = default;
                ${className}& operator=(const ${className}& rhs) noexcept = default;
                ${className}& operator=(${className}&& rhs) noexcept = default;
...
function formatWriterPropertyImpl(property) {
  ...
  const isVector = type.startsWith("std::vector") || type.startsWith("std::array");
