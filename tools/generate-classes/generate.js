...
function generate(options, schema) {
  ...
  let writerHeaders = lodash.uniq([
    getWriterIncludeFromName(base),
    `<${namespace}/${name}.h>`,
    ...lodash.flatten(properties.map((property) => property.writerHeaders)),
  ]);

  // Prevent header from including itself for recursive types like Tile
  writerHeaders = writerHeaders.filter((writerHeader) => {
    return writerHeader !== `"${name}JsonHandler.h"`;
  });

  writerHeaders.sort();

  const writerLocalTypes = lodash.uniq(
    lodash.flatten(properties.map((property) => property.writerLocalTypes))
  );

  const baseWriter = getWriterName(base);
  ...
  const writerHeader = `
        ...
        ${writerHeaders.map((header) => `#include ${header}`).join("\n")}
        ...
        namespace ${namespace} {
          class ${name}JsonHandler : public ${baseWriter}${thisConfig.extensionName ? `, public CesiumJsonWriter::IExtensionJsonHandler` : ""} {
          public:
            using ValueType = ${name};

            ${thisConfig.extensionName ? `static inline constexpr const char* ExtensionName = "${thisConfig.extensionName}";` : ""}

            ${name}JsonHandler(const CesiumJsonWriter::ExtensionWriterContext& context) noexcept;
            void reset(IJsonHandler* pParentHandler, ${name}* pObject);
            ...
            virtual IJsonHandler* writeNull() override {
              return ${baseWriter}::writeNull();
            };
            virtual IJsonHandler* writeBool(bool b) override {
              return ${baseWriter}::writeBool(b);
            }
            virtual IJsonHandler* writeInt32(int32_t i) override {
              return ${baseWriter}::writeInt32(i);
            }
            virtual IJsonHandler* writeUint32(uint32_t i) override {
              return ${baseWriter}::writeUint32(i);
            }
            virtual IJsonHandler* writeInt64(int64_t i) override {
              return ${baseWriter}::writeInt64(i);
            }
            virtual IJsonHandler* writeUint64(uint64_t i) override {
              return ${baseWriter}::writeUint64(i);
            }
            virtual IJsonHandler* writeDouble(double d) override {
              return ${baseWriter}::writeDouble(d);
            }
            virtual IJsonHandler* writeString(const std::string_view& str) override {
              return ${baseWriter}::writeString(str);
            }
            virtual IJsonHandler* writeObjectStart() override {
              return ${baseWriter}::writeObjectStart();
            }
            virtual IJsonHandler* writeObjectEnd() override {
              return ${baseWriter}::writeObjectEnd();
            }
            virtual IJsonHandler* writeArrayStart() override {
              return ${baseWriter}::writeArrayStart();
            }
            virtual IJsonHandler* writeArrayEnd() override {
              return ${baseWriter}::writeArrayEnd();
            }
            virtual void reportWarning(const std::string& warning, std::vector<std::string>&& context = std::vector<std::string>()) override {
              ${baseWriter}::reportWarning(warning, std::move(context));
            }
            ` : ""}

          protected:
            IJsonHandler* writeObjectKey${name}(const std::string& objectType, const std::string_view& str, ${name}& o);

          private:
            ${indent(writerLocalTypes.join("\n\n"), 12)}

            ${name}* _pObject = nullptr;
            ${indent(
              properties
                .map((property) => formatWriterProperty(property))
                .join("\n"),
              12
            )}
          };
  ...
  const writerHeaderOutputDir = path.join(writerOutputDir, "generated");
  ...
  const writerLocalTypesImpl = lodash.uniq(
    lodash.flatten(properties.map((property) => property.writerLocalTypesImpl))
  );

  const writerHeadersImpl = lodash.uniq([
    ...lodash.flatten(properties.map((property) => property.writerHeadersImpl)),
  ]);
  writerHeadersImpl.sort();

  function generateWriterOptionsInitializerList(properties, varName) {
    const initializerList = properties
      .filter((p) => p.writerType.toLowerCase().indexOf("jsonhandler") != -1)
      .map(
        (p) => `_${p.name}(${p.schemas && p.schemas.length > 0 ? varName : ""})`
      )
      .join(", ");
  ...
  const writerImpl = `
        ...
        #include "${name}JsonHandler.h"
        #include <${namespace}/${name}.h>
        ${writerHeadersImpl.map((header) => `#include ${header}`).join("\n")}
        #include <cassert>
        #include <string>

        using namespace ${namespace};

        ${name}JsonHandler::${name}JsonHandler(const CesiumJsonWriter::ExtensionWriterContext& context) noexcept : ${baseWriter}(context)${generateWriterOptionsInitializerList(properties, 'context')} {}

        void ${name}JsonHandler::reset(CesiumJsonWriter::IJsonHandler* pParentHandler, ${name}* pObject) {
          ${baseWriter}::reset(pParentHandler, pObject);
          this->_pObject = pObject;
        }

        CesiumJsonWriter::IJsonHandler* ${name}JsonHandler::writeObjectKey(const std::string_view& str) {
          assert(this->_pObject);
          return this->writeObjectKey${name}(${name}::TypeName, str, *this->_pObject);
        }

        ${thisConfig.extensionName ? `
        void ${name}JsonHandler::reset(CesiumJsonWriter::IJsonHandler* pParentHandler, CesiumUtility::ExtensibleObject& o, const std::string_view& extensionName) {
          std::any& value =
              o.extensions.emplace(extensionName, ${name}())
                  .first->second;
          this->reset(
        ...
        CesiumJsonWriter::IJsonHandler* ${name}JsonHandler::writeObjectKey${name}(const std::string& objectType, const std::string_view& str, ${name}& o) {
          using namespace std::string_literals;
          ...
          return this->writeObjectKey${removeNamespace(base)}(objectType, str, *this->_pObject);
        }

        ${indent(writerLocalTypesImpl.join("\n\n"), 8)}
  `;
  ...
  if (options.oneHandlerFile) {
    ...
    fs.appendFileSync(writerSourceOutputPath, unindent(writerImpl), "utf-8");
  } else {
    const writerSourceOutputPath = path.join(
      writerHeaderOutputDir,
      name + "JsonHandler.cpp"
    );
    fs.writeFileSync(writerSourceOutputPath, unindent(writerImpl), "utf-8");
  }
