...
public:
   ...
   * {@link CesiumJsonWriter::JsonHandler}.
  ...
  void registerExtension(const std::string& extensionName) {
    ...
    it->second.insert_or_assign(
        TExtended::TypeName,
        ExtensionWriterFactory([](const ExtensionWriterContext& context) {
  ...
  void registerExtension() {
    ...
    it->second.insert_or_assign(
        TExtended::TypeName,
        ExtensionHandlerFactory([](const ExtensionWriterContext& context) {
          return std::make_unique<TExtensionHandler>(context);
        }));
  }
   ...
   * encountered in the source JSON, it is write into a statically-typed
   ...
   * it will be write into a {@link CesiumUtility::JsonValue} even if a
