...
template <typename T> class CESIUMASYNC_API SharedFuture final {
public:
  SharedFuture(const SharedFuture& rhs)
      : _system(rhs._system),
        _slot(rhs._slot),
        _signal(
            rhs._signal ? orkester_shared_future_clone(rhs._signal) : nullptr) {
  }

  SharedFuture& operator=(const SharedFuture& rhs) {
    if (this != &rhs) {
      if (_signal)
        orkester_shared_future_drop(_signal);
      _system = rhs._system;
      _slot = rhs._slot;
      _signal =
          rhs._signal ? orkester_shared_future_clone(rhs._signal) : nullptr;
...
private:
  ...
  template <typename V>
  friend orkester_future_t CesiumImpl::stealSignal(SharedFuture<V>&);
  template <typename V>
  friend std::shared_ptr<CesiumImpl::ValueSlot<V>>
  CesiumImpl::getSlot(SharedFuture<V>&);
  ...
  template <typename Func> auto chainInPool(const ThreadPool& pool, Func&& f) {
    ...
    auto* adapter = new Adapter{
        ...
        outPromise,
        _system};
    orkester_shared_future_then_in_pool(
        _signal,
        pool._pool,
        &Adapter::invoke,
        adapter);
  ...
  Future<T> doCatch(orkester_context_t context, Func&& f) {
    ...
    auto* adapter =
        new CatchAdapter{_slot, nextSlot, std::forward<Func>(f), outPromise};
...
template <> inline void SharedFuture<void>::wait() const {
  const char* errPtr = nullptr;
  size_t errLen = 0;
  bool ok = orkester_shared_future_wait(_signal, &errPtr, &errLen);
  if (!ok && errPtr) {
