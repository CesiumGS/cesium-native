...
///
/// Holds a shared_ptr<ValueSlot<T>> (C++ value) and a orkester_promise_t (Rust signal).
...
template <typename T>
class CESIUMASYNC_API Promise final {
public:
    ...
    template <typename E, std::enable_if_t<!std::is_same_v<std::decay_t<E>, std::exception_ptr>, int> = 0>
...
private:
    ...
    struct State {
        ...
        ~State() noexcept {
            if (promise) orkester_promise_drop(promise);
        }
    };

    Promise(const orkester_async_t* system,
            std::shared_ptr<CesiumImpl::ValueSlot<T>> slot,
...
template <>
class CESIUMASYNC_API Promise<void> final {
public:
    Promise(const Promise&) = default;
    Promise& operator=(const Promise&) = default;
    Promise(Promise&&) noexcept = default;
