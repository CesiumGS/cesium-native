...
/// Shared implementation types for the orkester-backed Future/SharedFuture/Promise.
/// This header is included by both Future.h and SharedFuture.h.
...
struct ValueSlot {
    ...
    void store(const T& value) { state.template emplace<T>(value); }
    void storeError(std::exception_ptr e) { state = e; }

    bool hasValue() const { return std::holds_alternative<T>(state); }
    bool hasError() const { return std::holds_alternative<std::exception_ptr>(state); }
...
std::exception_ptr extractError(const ValueSlot<T>& slot) {
    if (auto* e = std::get_if<std::exception_ptr>(&slot.state)) return *e;
    return {};
}
...
struct ContinuationReturnType<Func, void> {
    using type = std::invoke_result_t<Func>;
};

template <typename T> struct RemoveFuture { using type = T; };
template <typename T> struct RemoveFuture<Future<T>> { using type = T; };
template <typename T> struct RemoveFuture<SharedFuture<T>> { using type = T; };
...
template <typename Func, typename T>
using ContinuationFutureType_t = Future<
