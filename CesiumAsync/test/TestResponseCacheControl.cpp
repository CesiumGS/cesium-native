...
TEST_CASE("Test parsing cache-control header") {
    SECTION("Header has no cache-control header") {
        ...
        std::optional<ResponseCacheControl> cacheControl = ResponseCacheControl::parseFromResponseHeaders(responseHeader);
        REQUIRE(cacheControl == std::nullopt);
    }

    SECTION("Header has cache-control header") {
        HttpHeaders responseHeader{
            {"Cache-Control", "Must-Revalidate, No-Cache, No-Store, No-Transform, Public, Private, Proxy-Revalidate, Max-Age = 1000,  S-Maxage = 10"},
        };
    ...
    SECTION("Header has cache-control header with only some directive") {
        HttpHeaders responseHeader{
            {"Cache-Control", "Must-Revalidate, No-Cache, No-Store, Public, Private, Max-Age = 1000,  S-Maxage = 10"},
        };
