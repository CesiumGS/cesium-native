...
TEST_CASE("GunzipAssetAccessor") {
    ...
    auto pAccessor = std::make_shared<GunzipAssetAccessor>(
        std::make_shared<MockAssetAccessor>(std::make_shared<MockAssetRequest>(
            ...
            std::make_unique<MockAssetResponse>(
                ...
                "Application/Whatever",
                HttpHeaders{std::make_pair("Some-Header", "in the response")},
                asBytes(std::vector<int>{0x1F, 0x8B, 0x08, 0x08, 0x34, 0xEE,
                                         0x77, 0x64, 0x00, 0x03, 0x6F, 0x6E,
                                         0x65, 0x74, 0x77, 0x6F, 0x74, 0x68,
                                         0x72, 0x65, 0x65, 0x2E, 0x64, 0x61,
                                         0x74, 0x00, 0x63, 0x64, 0x62, 0x06,
                                         0x00, 0x1D, 0x80, 0xBC, 0x55, 0x03,
                                         0x00, 0x00, 0x00})))));
  ...
  SUBCASE(
      "passes through a response that has a gzip header but can't be "
      "gunzipped") {
    auto pAccessor = std::make_shared<GunzipAssetAccessor>(
        std::make_shared<MockAssetAccessor>(std::make_shared<MockAssetRequest>(
            "GET",
