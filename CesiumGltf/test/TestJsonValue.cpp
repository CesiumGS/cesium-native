...
TEST_CASE("JsonValue does not have precision loss when storing / retreving numbers") {
...
TEST_CASE("JsonValue::getSafeNumber() throws if narrowing conversion error would occur") {
...
TEST_CASE("JsonValue::getSafeNumberOrDefault() returns default if narrowing conversion error would occur") {
    SECTION("2^64 - 1 cannot be converted back to a double") {
        auto value = JsonValue(std::numeric_limits<std::uint64_t>::max());
